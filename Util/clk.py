...
for index in range(len(exact_prescalers)):
    ...
    for rowindex in range(len(rows)):
        df = pd.concat([df, pd.DataFrame(np.array(rows[rowindex]).reshape(1, 4), columns=df.columns)])
...
df = pd.concat([df, pd.DataFrame(np.array(close_prescalers).reshape(len(close_prescalers), 4), columns=df.columns)])
